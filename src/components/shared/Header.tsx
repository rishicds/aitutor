...
const IconSideNav = () => {
  return (
    <div className="fixed left-0 top-0 h-screen bg-slate-900 text-slate-100 shadow-lg z-50">
      <SideNav />
    </div>
  );
};
...
const SideNav = () => {
  ...
  return (
    <nav className="h-full w-20 bg-slate-950 p-4 flex flex-col items-center gap-4">
      {/* 🔵 Logo */}
      <Link href="/">
        <svg
          width="40"
      ...
      <div className="mt-auto">
        {user ? (
          <button
            ...
            className="p-3 text-xl bg-red-600 hover:bg-red-500 rounded-md transition-colors"
          >
            <FiLogOut />
          </button>
        ) : (
          <Link href="/signin">
            <motion.button
              className="p-3 text-xl bg-green-600 hover:bg-green-500 rounded-md transition-colors"
