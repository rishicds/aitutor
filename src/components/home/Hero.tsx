...
export const Hero = () => {
  ...
  return (
    <section ref={targetRef} className="h-[100vh] text-gray-900 overflow-hidden">
      ...
      >
        <AnimatePresence>{isLoaded && <Nav mouseXSpring={mouseXSpring} mouseYSpring={mouseYSpring} />}</AnimatePresence>
        <AnimatePresence>
          {isLoaded && <CenterCopy mouseXSpring={mouseXSpring} mouseYSpring={mouseYSpring} />}
        </AnimatePresence>
        <AnimatePresence>{isLoaded && <ScrollArrow />}</AnimatePresence>
      </motion.div>
    </section>
  )
}
...
const Logo = ({ mouseXSpring, mouseYSpring }: { mouseXSpring: any; mouseYSpring: any }) => {
  return (
    <motion.div className="text-xl font-light tracking-wide relative" style={{ x: mouseXSpring, y: mouseYSpring }}>
