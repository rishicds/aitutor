...
export async function POST(request: NextRequest) {
  try {
    const { roadmapId, topicId, userId, completed } = await request.json();
    
    if (!roadmapId || !topicId || !userId || completed === undefined) {
      return NextResponse.json({
        error: 'Missing required parameters. Please provide roadmapId, topicId, userId, and completed status.'
      }, { status: 400 });
    }
